...
try {
    ...
    function Log-Step($message) {
     ...
     Log-Step "Starting RAM dump..."
     if (Test-Path $winpmem) {
         & $winpmem "$evidencePath\memory.raw"
         Log-Step "RAM saved to evidence\memory.raw"
     } else {
...
# Processes
Log-Step "Collecting running processes..."
$processes = Get-Process | Select-Object Name, Id, CPU, Path
$processes | Export-Csv "$evidencePath\processes.csv" -NoTypeInformation
Log-Step "Saved processes to evidence\processes.csv"

# Local Users
Log-Step "Collecting user accounts..."
...
Log-Step "Saved users to evidence\users.csv"

# Network TCP Connections
Log-Step "Collecting TCP connections..."
...
Log-Step "Saved TCP connections to evidence\network_tcp.csv"

# Network Neighbors
Log-Step "Collecting network neighbors..."
...
Log-Step "Saved network neighbors to evidence\network_neighbors.csv"

# Prefetch Files
Log-Step "Collecting prefetch files..."
...
Log-Step "Saved prefetch list to evidence\prefetch.csv"
...
Log-Step "HTML report created with detailed data at evidence\forensic_report.html"
}
finally {
    Stop-Transcript
